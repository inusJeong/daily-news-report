"""실행 사이에 기억해야 하는 것: 용어 커서, 최근 보낸 이슈(재방송 방지), 마지막 발송일(중복 발송 방지)."""
...
def record_sent(state: dict, today: date, titles: list[str], next_cursor: int, keep_days: int) -> dict:
    ...
    return {"term_cursor": next_cursor, "last_sent": today.isoformat(), "history": history}
